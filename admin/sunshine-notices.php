...
function sunshine_admin_notices() {
	...
	if ( ( current_time('timestamp') - $install_time ) >= DAY_IN_SECONDS * 20 ) {
	...
	if ( !$sunshine->is_pro() && ( ( current_time('timestamp') - $install_time ) >= DAY_IN_SECONDS * 10 ) ) {
	...
	if ( !$sunshine->is_pro() && ( ( current_time('timestamp') - $install_time ) >= DAY_IN_SECONDS * 45 ) ) {
		...
		$discount_text = '<p>You having been using Sunshine Photo Cart for quite a while and that\'s great! However, it appears you are not enjoying the awesome features of having a Sunshine Pro license (all the add-ons and 1-on-1 priority support).</p><p><strong>I am doing a one-time only offer of 50% off a Sunshine Pro license!</strong> Sunshine <em>rarely</em> does any kind of discounts, especially this big. You can be sure this offer will not happen again.</p>
						' . $has_addons . '
	    				<p><a href="https://www.sunshinephotocart.com/checkout/?edd_action=add_to_cart&download_id=44&discount=3VhPKwX0lJCe" target="_blank" class="button-primary notice-dismiss-button">Upgrade me please!</a> &nbsp; 
						<a href="https://www.sunshinephotocart.com/pro/?discount=3VhPKwX0lJCe" class="button" target="_blank">Learn more about Pro</a>
